...
function captain::cellar() {
  log::info "[CAPTAIN] [wine] Selling all wine worth max value {{{ | max_wine_value='$FARMRPG_MAX_WINE_SILVER'"
  cellar::sell_wine::all
  log::info "[CAPTAIN] [wine] Making sure we have max wine in cellar | max_wine='$FARMRPG_MAX_WINE'"
  cellar::store_wine::all
  log::info "[CAPTAIN] [wine] Celler is in optimal state }}}"
}
